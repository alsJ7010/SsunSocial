$(document).ready(function() {	
	...
	if(document.location.href == "http://localhost:8080/ssun/notice") {
		setUserInfo();
		setAdmin();
		showWritingArea();
		getAllNotice();
	...
	$("#ossn-wall-post-up").on("click", function(e) {
		...
		$.ajax({
			url : 'http://localhost:8080/ssun/post',
			method : 'POST',
			data : formData,
			processData : false,
			contentType : false,
			success : function(data) {
				...
				console.log("user_num value: " + $("#user_num").val());
			    ...
			    if(user_img=="" || user_img==null || myObj.anonymity=="Y") {
			    	...
			    	txt += "src='http://localhost:8080/ssun/resources/userImage/"+user_img+"'>";
			    }
			    ...
			    if(myObj.save_filename.length == 0) {
			    	...
			    	for(key in myObj.save_filename){
			    		...
			    		if(ext == "video") {
			    			...
			    			txt += "<source src=http://localhost:8080/ssun/resources/uploadFile/"+fileName+" type=video/"+cmpExtension(fileName)+"></video>";
			    		} else {
			    			txt += "<img src=http://localhost:8080/ssun/resources/uploadFile/"+fileName+"></div>";
			    		}
			    	}
			    }         
	...
	$("#notice-posting-btn").on("click", function(e) {
		...
		$.ajax({
			url : 'http://localhost:8080/ssun/postNotice',
			method : 'POST',
			data : formData,
			processData : false,
			contentType : false,
...
function getAllWriting() {
	...
	$.ajax({
		url:'http://localhost:8080/ssun/postList',
		method:'POST',
		...
		success:function(data){
			...
			for (x in myObj) {
	            ...
	            if(myObj[x].user_img=="" || myObj[x].user_img==null || myObj[x].anonymity=="Y") {
	            	...
	            	txt += "src=http://localhost:8080/ssun/resources/userImage/"+myObj[x].user_img+">";
	            }
	            ...
	            if(myObj[x].comment_num == 0) {
	            	...
	            	console.log("댓글몇개게!: "+ myObj[x].comment_num);
...
function showCommentBox(w_num){
	...
	if (document.getElementById("comments-list-"+w_num).style.display == "none") {
		...
		$.ajax({
			url : 'http://localhost:8080/ssun/commentList',
			method : 'POST',
			...
			success : function(data) {
				...
				for (x in myObj) {
					...
					if(myObj[x].user_num == user_num || user_num == 1000000000) {
...
function postComment(w_num){
	...
	if(window.event.keyCode == 13) {
		...
		if (window.event.shiftKey === false){
			...
			$.ajax({
				url : 'http://localhost:8080/ssun/writeComment',
				method : 'POST',
...
function loadWriting() {
	...
	$(window).scroll(function() {
		...
		if(($(document).height() - $(window).height() - 1 <= $(window).scrollTop()) && ($(document).height() - $(window).height() + 1 >= $(window).scrollTop())) {
			...
			$.ajax({
				url : 'http://localhost:8080/ssun/extraPost',
				method : 'POST',
				data : JSON.stringify(sendData),
				dataType : 'json',
				processData : false,
...
function deletePost(w_num) {
	...
	$.ajax({
		url:'http://localhost:8080/ssun/deletePost',
		method:'POST',
...
function deleteComment(c_num) {
	...
	$.ajax({
		url:'http://localhost:8080/ssun/deleteComment',
		method:'POST',
		data:JSON.stringify(del_num),
		processData:false,
		contentType:'application/json',
...
function editPost_getContent(w_num) {
	...
	$.ajax({
		url:'http://localhost:8080/ssun/getPostContent',
		method:'POST',
		data:JSON.stringify(sendData),
		processData:false,
		contentType:'application/json',
...
function editPost() {
	...
	$.ajax({
		url:'http://localhost:8080/ssun/editPost',
		method:'POST',
		data:JSON.stringify(sendData),
		processData:false,
		contentType:'application/json',
...
function editComment_getContent(c_num) {
	...
	$.ajax({
		url:'http://localhost:8080/ssun/getCommentContent',
		method:'POST',
		data:JSON.stringify(sendData),
		processData:false,
		contentType:'application/json',
...
function editComment() {
	...
	$.ajax({
		url:'http://localhost:8080/ssun/editComment',
		method:'POST',
		data:JSON.stringify(sendData),
		processData:false,
		contentType:'application/json',
...
function clickPostLike(w_num) {
	...
	$.ajax({
		url:'http://localhost:8080/ssun/postLike',
		method:'POST',
...
function clickCommentLike(c_num) {
	...
	$.ajax({
		url:'http://localhost:8080/ssun/commentLike',
		method:'POST',
		data:JSON.stringify(data),
		processData:false,
		contentType:'application/json',
...
function searchPost(){
	if(event.keyCode == 13) {
		...
		if(window.location.href == "http://localhost:8080/ssun/notice") {
			dep_code = "N";
		...
		window.location.href = "http://localhost:8080/ssun/searchPost?searchType="+selectBox+"&searchWord="+encodeURI(encodeURIComponent(search_word))+"&dep="+dep_code;
...
function setAdmin() {
	...
	if(user_num == 1000000000) {
		var txt = "<li><a class='' href='http://localhost:8080/ssun/admin/users'><i class='fa fa-user fa-lg'></i>관리자 페이지</a></li>"
		$('#sidebar-menu').append(txt);

		var url = window.location.href;
		if(url == "http://localhost:8080/ssun/mypage") {
			$('#deleteUser').remove();
		}
	}
}
...
function showWritingArea() {
	...
	if(user_num == 1000000000) {
		...
		txt += "<input type='hidden' id='user_num' name='user_num' value='"+user_num+"'>";
...
function putUpNotice(w_num) {
	...
	$.ajax({
		url:'http://localhost:8080/ssun/putUpNotice',
		method:'POST',
		data:JSON.stringify(sendData),
		processData:false,
		contentType:'application/json',
...
function putDownNotice(w_num) {
	...
	$.ajax({
		url:'http://localhost:8080/ssun/putDownNotice',
		method:'POST',
		data:JSON.stringify(sendData),
		processData:false,
		contentType:'application/json',
...
function getNoticeInfo() {
	...
	$.ajax({
		url:'http://localhost:8080/ssun/noticePostList',
		method:'POST',
		processData:false,
		contentType:false,
		success:function(data){
...
function bestLikePostInfo() {
	...
	$.ajax({
		url:'http://localhost:8080/ssun/bestLikePostInfo',
		method:'POST',
		processData:false,
		contentType:false,
		success:function(data){
...
function bestCommentPostInfo() {
	...
	$.ajax({
		url:'http://localhost:8080/ssun/bestCommentPostInfo',
		method:'POST',
		processData:false,
		contentType:false,
		success:function(data){
