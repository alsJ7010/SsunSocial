...
function searchPost(){
	...
	$.ajax({
		url:'http://localhost:8080/ssun/search',
		method:'POST',
		data:JSON.stringify(sendData),
		processData:false,
		contentType:'application/json',
...
function search() {
	if(event.keyCode == 13) {
		...
		window.location.replace("http://localhost:8080/ssun/searchPost?searchType="+selectBox+"&searchWord="+encodeURI(encodeURIComponent(search_word))+"&dep="+dep_code);
	}
}
...
function loadSearchWriting() {
	$(window).scroll(function() {
		...
		if(($(document).height() - $(window).height() - 1 <= $(window).scrollTop()) && ($(document).height() - $(window).height() + 1 >= $(window).scrollTop())) {
			...
			$.ajax({
				url : 'http://localhost:8080/ssun/searchExtraPost',
				method : 'POST',
				data : JSON.stringify(sendData),
				dataType : 'json',
				processData : false,
